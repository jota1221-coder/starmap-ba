...
### 4️⃣ Nice to have
- CSP completa en Report-Only → enforce (B3) · `npm audit fix` (B4) · región iad1 explícita (M16) · timeout Open-Meteo a 3-4s (M14) · `React.cache` en getPointBySlug (M15) · eliminar Geist Mono (B15) · `sizes` en Figure (B16) · Cache-Control immutable (B17) · índices de FKs + limpieza de sesiones (B7, B8) · pending state del login (M26) · resto de Bajos de UI/UX y código (B18-B32)
